...
@Service
public class AlgoSell implements IStrategy {
    ...
    public Instrument instrument = Instrument.AUDCAD;
    @Configurable("Period")
    ...
    public void onTick(Instrument inst, ITick tick) throws JFException {
        ...
        try {
            if (getPositions().size() < 10 && inst.isTradable()) {

                if (pendingOrderSell != null && pendingOrderSell.getState() == IOrder.State.FILLED) {
                    stopLossPrice = tick.getAsk() - getPipPrice(LOSS_PIP);
                    engine.submitOrder(getLabel(inst), inst, OrderCommand.SELL, ORDER_AMOUNT, tick.getAsk(), 0, stopLossPrice, tick.getBid());
