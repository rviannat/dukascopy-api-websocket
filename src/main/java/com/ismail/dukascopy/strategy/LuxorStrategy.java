...
import com.ismail.dukascopy.model.*;
...
@Service
@Slf4j
public class LuxorStrategy implements IStrategy {
	...
	public void onStart(IContext context) {
		...
		List<String> instrumentIDList =
				DukasUtil.splitToArrayList(config.getSubscriptionInstruments(), ',');
	...
	public void onTick(Instrument instrument, ITick tick) {
		...
		if (tobSubscribersSize > 0) {
			...
			int depthLevels = (tick.getBids() == null || tick.getAsks() == null) ? 0
			...
			if (depthLevels > 0) {
				...
				for (int i = 0; i < depthLevels; i++) {
					OrderBookEntry bidEntry =
							new OrderBookEntry(tick.getBidVolumes()[i], tick.getBids()[i]);
					st.bids.add(bidEntry);

					OrderBookEntry askEntry =
							new OrderBookEntry(tick.getAskVolumes()[i], tick.getAsks()[i]);
	 ...
	 * @throws Exception
	 */
	public IOrder openPosition(String clientOrderID, Instrument instrument, OrderSide orderSide,
			OrderType orderType, double quantity, double price, double slippage,
			double takeProfitPips, double stopLossPips, boolean trailingSl, long timeout)
			throws Exception {
		if (context == null)
			throw new RuntimeException("Strategy context not initialized yet");
		...
		OpenPositionTask task = new OpenPositionTask(clientOrderID, instrument, cmd,
				quantityNormalized, price, slippage, takeProfitPips, stopLossPips, trailingSl);

		Future<IOrder> order = context.executeTask(task);
	...
	public class OpenPositionTask implements Callable<IOrder> {
		...
		public OpenPositionTask(String clientOrderID, Instrument instrument, OrderCommand cmd,
				double quantity, double price, double slippage, double takeProfitPips,
				double stopLossPips, boolean trailingSl) {
		...
		public IOrder call() throws Exception {
			IOrder order =
					engine.submitOrder(clientOrderID, instrument, cmd, quantity, price, slippage);

			try {
				order.waitForUpdate(2000, IOrder.State.FILLED);

				double entryPrice = price > 0L ? price : order.getOpenPrice();
				if (order.getState() == IOrder.State.FILLED
						|| order.getState() == IOrder.State.OPENED) {
					if (order.isLong()) {
						double takeProfitPrice =
								entryPrice + (takeProfitPips * instrument.getPipValue());
						...
						if (trailingSl) {
							...
							if (stopLossPips > 0L) {
								order.setStopLossPrice(stopLossPrice, OfferSide.ASK, stopLossPips);
							}
						} else {
							if (takeProfitPips > 0L) {
								order.setTakeProfitPrice(takeProfitPrice);
	 ...
	 * @throws Exception
	 */
	public IOrder editPosition(Optional<String> clientOrderID, Optional<String> dukasOrderID,
			double takeProfitPrice, double stopLossPrice, double takeProfitPips,
			double stopLossPips, boolean trailingSl, long timeout) throws Exception {
		if (context == null)
			throw new RuntimeException("Strategy context not initialized yet");
		...
		// Programming
		// So we submit a task; then wait for it to get done
		EditPositionTask task = new EditPositionTask(order, takeProfitPrice, stopLossPrice,
				takeProfitPips, stopLossPips, trailingSl);
		context.executeTask(task);

		Future<IOrder> updatedOrder = context.executeTask(task);

		while (!updatedOrder.isDone()) {
			Thread.sleep(100);
		}

		return updatedOrder.get();
		/*
		 * // we have to wait for a given timeout long sleepTime = 100; int iterations = (int)
		 * (timeout / sleepTime);
		 * 
		 * for (int i = 0; i < iterations; i++) { // sleep for a minimal amount at a time; so we can
		 * get the result quick when the // task is done try { Thread.sleep(sleepTime); } catch
		 * (InterruptedException ie) {}
		 * 
		 * if (task.taskDone) { if (task.rejectReason != null) { log.info("rejectReason: " +
		 * task.rejectReason); throw new RuntimeException(task.rejectReason); } else { return
		 * task.order; } } }
		 * 
		 * // throw new RuntimeException("Timeout editing order"); return null;
		 */
	}
	...
	public class EditPositionTask implements Callable<IOrder> {
		...
		public EditPositionTask(IOrder order, double takeProfitPrice, double stopLossPrice,
				double takeProfitPips, double stopLossPips, boolean trailingSl) {
			this.order = order;
		...
		public IOrder call() throws Exception {
			try {
				...
				if (order.isLong()) {
					double tpPrice = entryPrice + (takeProfitPips * instrument.getPipValue());
					double slPrice = entryPrice - (stopLossPips * instrument.getPipValue());
					...
					if (trailingSl) {
						...
						if (stopLossPips > 0L) {
							order.setStopLossPrice(slPrice, OfferSide.BID, stopLossPips);
						}
					} else {
						if (takeProfitPips > 0L) {
							order.setTakeProfitPrice(tpPrice);
					...
					double tpPrice = takeProfitPrice > 0L ? takeProfitPrice
							: entryPrice - (takeProfitPips * instrument.getPipValue());
					...
					if (trailingSl) {
						...
						if (stopLossPips > 0L) {
							order.setStopLossPrice(slPrice, OfferSide.ASK, stopLossPips);
						}
					} else {
						if (takeProfitPips > 0L) {
							order.setTakeProfitPrice(tpPrice);
	 ...
	 * @throws Exception
	 */
	public IOrder closePosition(Optional<String> clientOrderID, Optional<String> dukasOrderID,
			double quantity, double price, double slippage, long timeout) throws Exception {
		if (context == null)
			throw new RuntimeException("Strategy context not initialized yet");
	 ...
	 * @throws Exception
	 */
	public List<IBar> getHistData(Instrument instrument, Period period, OfferSide pOfferSide,
			long timeFrom, long timeTo) throws Exception {
